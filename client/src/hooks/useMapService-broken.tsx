...
export function MapServiceProvider({ children }: { children: ReactNode }) {
  ...
  // 키워드로 장소 검색 (실제 카카오 API 사용)
  const searchPlacesByKeyword = useCallback(async (keyword: string, options?: {
    radius?: number;
    page?: number;
    size?: number;
  }): Promise<Place[]> => {
    ...
    try {
      const searchOptions = {
        ...
        radius: options?.radius || searchRadius,
        page: options?.page || 1,
        size: options?.size || 15,
        sort: 'distance' as const
      };

      const places = await kakaoMapService.searchByKeyword(keyword, searchOptions);
      ...
      toast({
        title: "장소 검색 실패",
        description: error.message || "장소 검색에 실패했습니다.",
        variant: "destructive"
      });
      
      return [];
    }
  }, [currentLocation, searchRadius, toast, applyFilters]);

  // 필터 적용 함수
  const applyFilters = useCallback((places: Place[]): Place[] => {
    let filteredPlaces = [...places];

    if (filterOptions.certifiedOnly) {
      filteredPlaces = filteredPlaces.filter(place => place.isCertified);
    }

    if (filterOptions.petFriendlyLevel) {
      filteredPlaces = filteredPlaces.filter(place => 
        place.petFriendlyLevel === filterOptions.petFriendlyLevel
      );
    }
    ...
    if (filterOptions.features && filterOptions.features.length > 0) {
      filteredPlaces = filteredPlaces.filter(place =>
        place.features && 
        filterOptions.features!.some(feature => place.features!.includes(feature))
      );
    }
  ...
  const searchNearbyPlaces = useCallback(async (
    ...
    try {
      ...
      const keyword = typeKeywords[type] || type;
      const places = await searchPlacesByKeyword(keyword, { radius });
  ...
  }, [currentLocation, searchRadius, toast, searchPlacesByKeyword]);
